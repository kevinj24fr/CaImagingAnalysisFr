#' Reporting Functions
#'
#' Automated reporting and quality control utilities for calcium imaging analysis.
#'
#' @name reporting
NULL
...
create_report_rmd <- function(analysis_results, title, include_code, include_plots, include_qc) {
  c(
    ...
    "cat(\"Package version:\", as.character(packageVersion(\"CaImagingAnalysisFr\")), \"\\n\")",
    "```",
    "",
    "# Data Summary",
    "",
    ...
    "    knitr::kable(params$analysis_results$data_summary, caption = \"Data Summary\")",
    "  } else {",
    "    print(params$analysis_results$data_summary)",
    "  }",
    "}",
    "```",
    "",
    ...
    if (include_qc) {
      c(
        ...
        "  if (requireNamespace(\"knitr\", quietly = TRUE)) {",
        "    knitr::kable(params$analysis_results$qc_metrics, caption = \"Quality Control Metrics\")",
        "  } else {",
        "    print(params$analysis_results$qc_metrics)",
        "  }",
        "}",
        "```",
        ""
    ...
    "  if (!is.null(params$analysis_results$spike_results$spikes)) {",
    "    spike_sum <- if (is.matrix(params$analysis_results$spike_results$spikes)) {",
    "      sum(params$analysis_results$spike_results$spikes)",
    "    } else if (is.numeric(params$analysis_results$spike_results$spikes)) {",
    "      sum(params$analysis_results$spike_results$spikes)",
    "    } else {",
    "      0",
    "    }",
    "    cat(\"Number of detected spikes:\", spike_sum, \"\\n\")",
    "  }",
    "  if (!is.null(params$analysis_results$spike_results$spike_rates)) {",
    "    mean_rate <- if (is.numeric(params$analysis_results$spike_results$spike_rates)) {",
    "      mean(params$analysis_results$spike_results$spike_rates, na.rm = TRUE)",
    "    } else {",
    "      0",
    "    }",
    "    cat(\"Mean spike rate:\", round(mean_rate, 3), \"Hz\\n\")",
    "  }",
    "}",
    "```",
    "",
    if (include_plots) {
      c(
        ...
        "  if (!is.null(params$analysis_results$plots$main_plot)) {",
    ...
    "  if (!is.null(params$analysis_results$statistical_analysis$summary)) {",
    "    if (requireNamespace(\"knitr\", quietly = TRUE)) {",
    "      knitr::kable(params$analysis_results$statistical_analysis$summary, caption = \"Statistical Summary\")",
    "    } else {",
    "      print(params$analysis_results$statistical_analysis$summary)",
    "    }",
    "  }",
    "}",
    "```",
...
create_pipeline_script <- function(analysis_results, include_comments, include_data_loading) {
  c(
    ...
    "qc_metrics <- calculate_correction_quality(raw_data, corrected_data)",
    "",
    "# Print Results",
